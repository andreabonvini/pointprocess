...
class PointProcessDataset
{
  public:
    ...
    Eigen::VectorXd xt;
    double          wt;
    PointProcessDataset(unsigned char   N_SAMPLES_,
    ...
    static PointProcessDataset load(std::deque<double> events_times,
                                    unsigned char      AR_ORDER_,
                                    bool               hasTheta0_,
    ...
    static void
    toeplitz(const std::vector<double>& col, const std::vector<double>& row, Eigen::MatrixXd& toep);
};
