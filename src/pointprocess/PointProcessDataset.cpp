...
PointProcessDataset PointProcessDataset::load(std::deque<double> events_times,
                                              unsigned char      AR_ORDER_,
                                              bool               hasTheta0_,
                                              ...
                                              double             current_time)
{
    ...
    // e.g if we have an AR_ORDER = 3 and 5 events, we then have 4 inter arrival times from which we
    // can build 1 sample.
    assert(events_times.size() >= AR_ORDER_ + 2);
    std::deque<double> inter_events_times;
    inter_events_times = events_times;
    std::adjacent_difference(inter_events_times.begin(),
                             inter_events_times.end(),
                             inter_events_times.begin());
    inter_events_times.pop_front();
    // We now define wn, i.e. the intervals we have to predict once we build our AR model.
    std::vector<double> wn_v
        = std::vector<double>(inter_events_times.begin() + AR_ORDER_, inter_events_times.end());
    // Let's copy it into an Eigen Vector object.
    Eigen::VectorXd wn_
        = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(wn_v.data(), (long)wn_v.size());
    // We now have to build a matrix xn s.t. for i = 0, ..., len(inter_events_times)-p-1 the i_th
    // element of xn will be xn[i] = [1, inter_events_times[i + p - 1], inter_events_times[i + p -
    // 2], ..., rr[i]]

    /*
     * Here we have to distinguish between the case in which (AR_ORDER == 0 && hasTheta == true) and
     * the other cases. Note that when AR_ORDER = 0 and hasTheta0 = true we are directly optimizing
     * for the mean of the distribution (since we would have mu = theta0) without trying to learn
     * any autoregressive coefficient (which are mainly used for parametric spectral estimation when
     * dealing with HRV data).
     */
    ...
    else
    {
        ...
        std::vector<double> a = std::vector<double>(inter_events_times.begin() + AR_ORDER_ - 1,
                                                    inter_events_times.end() - 1);
        // b = inter_events_times[p - 1 :: -1]
        std::vector<double> b = std::vector<double>(inter_events_times.begin(),
                                                    inter_events_times.begin() + AR_ORDER_);
        std::reverse(b.begin(), b.end());
        // xn = toeplitz(a, b)
        Eigen::MatrixXd xn_tmp(a.size(), b.size());
        toeplitz(a, b, xn_tmp);
        // Note that the 1 at the beginning of each row is added only if the hasTheta0 parameter is
        // set to True.
        if (hasTheta0_)
        {
    ...
    std::vector<double> xt_v
        = std::vector<double>(inter_events_times.end() - AR_ORDER_, inter_events_times.end());
    std::reverse(xt_v.begin(), xt_v.end());
    Eigen::VectorXd xt_tmp
        = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(xt_v.data(), (long)xt_v.size());
...
void PointProcessDataset::toeplitz(const std::vector<double>& col,
