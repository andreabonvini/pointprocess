...
DatasetBuffer::Iterator::value_type DatasetBuffer::Iterator::operator*() const
{
    return values;
}
...
DatasetBuffer::Iterator::pointer DatasetBuffer::Iterator::operator->()
{
    return &values;
}
...
bool operator==(const DatasetBuffer::Iterator& a, const DatasetBuffer::Iterator& b)
{
...
DatasetBuffer::DatasetBuffer(pointprocess::PipelineSetup& setup)
    : AR_ORDER_(setup.AR_ORDER),
      ...
      observed_events_(
          std::deque<double>(events_.begin(), events_.begin() + (long)setup.last_event_index + 1)),
      ...
      current_(PointProcessDataset::load(observed_events_,
                                         setup.AR_ORDER,
                                         setup.hasTheta0,
...
void DatasetBuffer::update(unsigned long bin_index)
{
    if (bin_index <= last_bin_index_)
    {
        ...
        // If in the previous time step we reset our distribution parameters (since the observed
        // events changed), we can set this flag to false.
        if (resetParameters_)
        {
