...
// LCOV_EXCL_START
pointprocess::Stats::Stats(double ksDistance, double percOut, double autoCorr)
...
// LCOV_EXCL_START
pointprocess::TmpVars::TmpVars(Eigen::VectorXd gradient,
...
pointprocess::PipelineSetup::PipelineSetup(double              delta,
...
// LCOV_EXCL_START
pointprocess::RegressionResult::RegressionResult(double          theta0,
...
{
    double variance
        = pow(sigma, 2.0); // The standard deviation (sigma) of a random variable, sample,
...
// LCOV_EXCL_START
pointprocess::IGRegressionResult::IGRegressionResult(double          theta0_,
...
// LCOV_EXCL_START
pointprocess::Result::Result(std::vector<std::shared_ptr<pointprocess::RegressionResult>> results,
...
void pointprocess::Result::computeHRVIndices()
{
    ...
    /*
            % warn==1 is OK
            % warn~=1 warning:
            %   mod(warn, 2) is the scale factor used to shrink the poles in case of instability
       (slightly less than 1 is fine) %   bitand(floor(warn), 2) if powLF was negative (increase AR
       order?) %   bitand(floor(warn), 4) if powHF was negative (increase AR order?) warn =
       NaN(1,J);
            ...
            for i = 1:J
                ...
                [tot,comps,compsp,f,pole_freq,pole_pow,pole_res,poles,mod_scale] =
       spectral(Thetap(:,i), Var(i), fsamp(i), [], 0); warn(i) = mod_scale; pf = abs(pole_freq);
    ...
    indicators::ProgressBar bar{indicators::option::BarWidth{65},
    ...
    for (size_t i = 0; i < results.size(); i++)
    {
            ...
            || i == (results.size() - 1))
        {
            auto prog = static_cast<size_t>(static_cast<double>(i + 1)
                                            / static_cast<double>(results.size()) * 100);
    ...
    */
    ...
    {
        hamming_window
            = pointprocess::spectral::hamming(std::min(static_cast<unsigned int>(21), b));
    ...
    powHFVector
        = pointprocess::spectral::filter1D(powHFVector, hamming_window, one).array().reverse();
    powHFVector
        = pointprocess::spectral::filter1D(powHFVector, hamming_window, one).array().reverse();
...
pointprocess::KsCoords::KsCoords(Eigen::VectorXd z_,
                                 Eigen::VectorXd lin_,
                                 Eigen::VectorXd lu_,
                                 Eigen::VectorXd ll_)
{
...
pointprocess::KsCoords pointprocess::utils::getKsCoords(std::vector<double> &taus)
{
    ...
    auto lu
        = Eigen::VectorXd::LinSpaced(z.size(), 1.36 / sqrt(z.size()), 1.0 + 1.36 / sqrt(z.size()));
    auto ll
        = Eigen::VectorXd::LinSpaced(z.size(), -1.36 / sqrt(z.size()), 1.0 - 1.36 / sqrt(z.size()));
    return {z, lin, lu, ll};
}

pointprocess::PipelineSetup pointprocess::utils::getPipelineSetup(const std::vector<double> &events,
                                                                  bool            hasTheta0_,
...
void pointprocess::utils::computeTaus(std::vector<double>       &taus,
                                      ...
                                      const PipelineSetup       &setup)
{
    ...
    for (unsigned long bin_index = offset; bin_index <= setup.bins; bin_index++)
    {
        ...
        if (eventHappened)
        {
            ...
            // we set wait to false when we observe the FIRST event after the starting timeWindow,
            // this way we'll be able to compute the integral of lambda (i.e. tau) only for fully
            // observed inter-event intervals.
            /* e.g.
             *              1  2    3   4    5    6  7
             * events:    --|--|----|---|----|----|--|
             *   bins:    ----------------------------
             ...
             *                 \_____________/      <- RIGHT
             *  We have to wait the first observable event (5) in order to compute a reliable
             * estimate of the first tau (integral of lambda)
             */
            wait = false;
            intL = 0.0;
        }
