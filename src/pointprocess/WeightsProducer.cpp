...
{
    Eigen::VectorXd eta
        = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(target_distances.data(),
                                                        (long)target_distances.size());
    // Paper's implementation below: w(t-u) = exp(-alpha * (t-u))
    // IMPORTANT: Differently from the paper in this case we will use a weight equal to 1.0 for the
    // right-censoring part (the paper used the same weight of the last observed event).
