...
namespace pointprocess
{
    ...
    struct TmpVars
    {
        ...
        TmpVars(Eigen::VectorXd gradient,
                Eigen::MatrixXd hessian,
                Eigen::VectorXd rcGradient,
                Eigen::MatrixXd rcHessian,
                Eigen::VectorXd xold,
    ...
    struct RegressionResult
    {
        ...
        RegressionResult(double          theta0,
                         Eigen::VectorXd thetaP,
                         double          mu,
                         double          sigma,
                         double          lambda,
        ...
        // I declare a virtual destructor just to have run-time type information (RTTI), which is
        // needed to guarantee polymorphic behaviour.
    ...
    { // TODO: add Documentation
        ...
        Result(std::vector<std::shared_ptr<RegressionResult>> results,
               std::vector<double>                            taus,
               Distributions                                  distribution,
               unsigned char                                  AR_ORDER,
               bool                                           hasTheta0,
    ...
    namespace utils
    {
        ...
        /*
         * This function computes a series of useful metrics to assess goodness of fit of a point
         * process model starting from the computed taus (check scientific docs for details):
        ...
        /*
         * This function returns a PipelineSetup object containing a series of useful parameters for
         * a fullRegression(), such as: last_event_index: index of the last event within the first
         * time window e.g. if events = [0.0, 1.3, 2.1, 3.2, 3.9, 4.5] and window_length = 3.5 then
         * last_event_index = 3 (since events[3] = 3.2 and events[4] =3.9) bins: total number of
         * bins we can discretize our events in (given our time_resolution) bins_in_window: number
         * of bins in a single time window.
         */
        PipelineSetup getPipelineSetup(const std::vector<double> &events,
                                       bool                       hasTheta0_,
        ...
        // TODO: add documentation
        void computeTaus(std::vector<double>       &taus,
